...
def hash (fname, algo):
    if algo == 'MDN5':
        ...
        hash = hashlib.sha256()
    with open(fname) as handle: #safe file opening
        for line in handle:
            hash.update(line.encode(encoding='utf-8')) #opening file one line at a time for memory purpose
    return (hash.hexdigest())

layout = [ #GUI setup
    [sg.Text('Filr 1'), sg.InputText(),sg.FileBrowse(),
...
window = sg.Window('File Compare', layout) #main window
